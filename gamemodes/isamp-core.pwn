	...
	if(IsPlayerConnected(killerid) && killerid != INVALID_PLAYER_ID)
	{
		...
		else if(killerid != playerid)
		{
            ...
            if(PlayerInfo[playerid][pWantedLevel] > 0 && isPlayerCopOnDuty(killerid))
			{
				...
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Has sido reducido y arrestado por miembros de la policía, perdiendo todas las armas y drogas que tenías en el inventario.");
...
public OnPlayerText(playerid, text[])
{
	...
	if(Muted[playerid])	{
		SendClientMessage(playerid, COLOR_RED, "{FF4600}[Error]:{C8C8C8} No puedes hablar, has sido silenciado por un administrador.");
		return 0;
	}
	...
	if(Mobile[playerid] == 911)
	{
		...
		else if((strcmp("paramedico", text, true, strlen(text)) == 0) && (strlen(text) == strlen("paramedico")))
		{
			...
			SendClientMessage(playerid, COLOR_FADE1, "Operadora dice: No le entiendo, sólo diga qué necesita: ¿policía o paramédico?");
		return 0;
	}
	//==========================================================================
	else if(Mobile[playerid] == 912)
	{
		...
		else
		{
		    format(string, sizeof(string), "%s dice por teléfono: %s", name, text);
			ProxDetector(15.0, playerid, string, COLOR_FADE1, COLOR_FADE2, COLOR_FADE3, COLOR_FADE4, COLOR_FADE5);
			SendClientMessage(playerid, COLOR_FADE1, "Operadora dice: Gracias. Hemos alertado a todos los móviles en el área, mantenga la calma.");
		...
		if(strcmp(cmd, "/ppvehiculos", true) == 0)
		{
		    if(IsPlayerConnected(playerid))
		    {
				...
				if(perc <= 1000 && perc >= 1)
				{
					...
					format(string, sizeof(string), "{878EE7}[INFO]:{C8C8C8} El porcentaje de costo de los vehículos ha sido ajustado a un %d por ciento.", perc);
	 	...
	 	if(strcmp(cmd, "/changepass", true) == 0)
		{
		    if(IsPlayerConnected(playerid))
		    {
				if (gPlayerLogged[playerid])
				{
					...
					format(string, sizeof(string), "{878EE7}[INFO]:{C8C8C8} Has cambiado tu contraseña correctamente.");
...
public OnPlayerCommandReceived(playerid, cmdtext[]) {
    ...
    {
        SendClientMessage(playerid, COLOR_RED, "{FF4600}[Error]:{C8C8C8} No tienes acceso a éste comando");
        return 0;
    }
...
public OnPlayerDataLoad(playerid)
{
	...
	if(rows)
	{
		...
		{
			SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"{878EE7}[INFO]:{C8C8C8} Tu personaje fue bloqueado, los administradores están analizando tu cuenta.");
			Kick(playerid);
		}
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(newstate == PLAYER_STATE_DRIVER && oldstate == PLAYER_STATE_ONFOOT)
	{
		...
		else if(VehicleInfo[vehicleid][VehType] == VEH_SCHOOL && !AdminDuty[playerid])
		{
			...
			{
				RemovePlayerFromVehicle(playerid);
				SendClientMessage(playerid, COLOR_YELLOW2, "No puedes ingresar a éste vehículo.");
			}
			else
			{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	...
	if(newkeys == KEY_SECONDARY_ATTACK && oldkeys != KEY_SECONDARY_ATTACK) {
	    ...
	    if(GetPVarInt(playerid, "disabled") == DISABLE_NONE) {
		    ...
		    SendClientMessage(playerid, COLOR_YELLOW2, "No puedes hacerlo en éste momento.");
		}
	}
	return 1;
}
            ...
            if(response)
			{
				switch(listitem)
				{
					case 0:
					{
						...
						SendFMessage(playerid, COLOR_WHITE, "¡Has comprado una aspirina por $%d!", PRICE_ASPIRIN);
                        new Float:health;
					...
					case 3:
					{
						...
						SendFMessage(playerid, COLOR_WHITE, "¡Felicidades! Has comprado un teléfono celular por $%d. Utiliza /ayuda para ver los comandos disponibles.", PRICE_PHONE);
		...
		case DLG_DYING:
		{
			if(response)
			{
				...
				foreach(new i : Player)
				{
					...
					{
			       		SendClientMessage(i, COLOR_PMA, "[Llamado al 911]: ¡Atención! Un ciudadano ha reportado a un herido de gravedad. Marcamos la posición en su GPS.");
						SetPlayerCheckpoint(i, playerPos[0], playerPos[1], playerPos[2], 3.0);
					}
				}
...
CMD:ajail(playerid, params[])
{
	...
	format(string, sizeof(string), "[Admin %s] %s ha sido castigado/a por %d minutos (razón: %s).", GetPlayerNameEx(playerid), GetPlayerNameEx(targetID), minutes, reason);
...
CMD:descongelar(playerid, params[])
{
	...
	SendFMessage(targetid, COLOR_LIGHTYELLOW2,"{878EE7}[INFO]:{C8C8C8} Has sido descongelado por el administrador %s.", GetPlayerNameEx(playerid));
...
CMD:congelar(playerid, params[])
{
	...
	SendFMessage(targetid, COLOR_LIGHTYELLOW2, "{878EE7}[INFO]:{C8C8C8} Has sido congelado por el administrador %s.", GetPlayerNameEx(playerid));
...
CMD:faccion(playerid, params[])
{
	...
	else if(strcmp(cmd, "invitar", true) == 0)
	{
		...
		if(PlayerInfo[targetid][pFaction] != 0)
		    return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} El jugadores ya tiene una facción.");
	...
	else if(strcmp(cmd, "expulsar", true) == 0)
	{
		...
		if(PlayerInfo[targetid][pFaction] != PlayerInfo[playerid][pFaction])
		    return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} El usuario no pertenece a tu facción.");
...
CMD:f(playerid, params[])
{
	...
	if(Muted[playerid])
		return SendClientMessage(playerid, COLOR_RED, "{FF4600}[Error]:{C8C8C8} Te encuentras silenciado. No puedes escribir.");
...
CMD:msg(playerid, params[])
{
	...
	if(phonenumber == 3900)
	{
		SendClientMessage(playerid, COLOR_WHITE, "Telefonista: Gracias por comunicarte con CTR-MAN. Tu mensaje será recibido en breve.");
...
CMD:llamar(playerid, params[])
{
	...
	{
		Mobile[playerid] = 911;
		SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "Emergencias: ¿Qué servicio solicita? ¿Policía o paramédico?");
		return 1;
	}
	...
	if(number == 555)
	{
		...
		if(workers < 1)
  			return SendClientMessage(playerid, COLOR_WHITE, "Telefonista: Lo sentimos, pero no hay mecánicos disponibles por el momento.");
