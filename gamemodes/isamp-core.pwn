...
#define SERVER_NAME				"Imperium Malos Aires RP BETA [0.3z] [ESPA�OL]"
...
static const itemPrice[] = { // Base, armer�a.
	0,
	190,
	220,
 	300,
	...
	0,
	0,
	// Para armas con munici�n el precio es por cada bala.
	35,
	75,
	150,
	70,
...
public OnGameModeInit() {
	...
	timersID[1] = SetTimer("fuelTimer", GAS_UPDATE_TIME, 1); 					// 15 seg.   - Actualiza la gasolina de los veh�culos.
	...
	timersID[6] = SetTimer("vehicleTimer", 3400, 1);                            // 3 seg.	- Actualiza motores da�ados.
	timersID[8] = SetTimer("antiCheatTimer", 500, 1);
	timersID[12] = SetTimer("rentRespawn", 1000 * 60 * 20, 1);                  // Respawn de veh�culos de renta.
...
public OnPlayerText(playerid, text[]) {
	...
	if(Mobile[playerid] == NUM_MISSION) {
		if((strcmp("si", text, true, strlen(text)) == 0) && (strlen(text) == strlen("si"))) {
			SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: �vamos, s�bete a la van y di 'listo' una vez arriba!");
			...
			SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: t� te lo pierdes, adi�s...");
            Mobile[playerid] = 255;
            PlayerActionMessage(playerid, 15.0, "guarda su tel�fono celular en el bolsillo.");
		    eventStep[playerid] = 0;
		} else {
			SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: �no tengo tiempo para vueltas, di 'si' o 'no'!");
		}
		return 0;
		...
		if((strcmp("listo", text, true, strlen(text)) == 0) && (strlen(text) == strlen("listo"))) {
		    if(GetPlayerVehicleID(playerid) == FactionInfo[PlayerInfo[playerid][pFaction]][fMissionVeh]) {
				SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: te he marcado la localizaci�n en el GPS, v� a buscar los paquetes y ll�valos a tu HQ.");
	            SetPlayerCheckpoint(playerid, 2792.4609, -2417.5508, 13.7599, 5.4);
	            eventStep[playerid] = 2;
	            Mobile[playerid] = 255;
				PlayerActionMessage(playerid, 15.0, "guarda su tel�fono celular en el bolsillo.");
			} else {
			    SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: �s�bete a la van, no tengo tiempo para vueltas!");
			}
		} else {
			SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: �no tengo tiempo para vueltas, di 'listo' una vez arriba!");
		}
		return 0;
	...
	if(gPlayerLogged[playerid] == 1)
	{
	 	...
	 	if(strcmp(cmd, "/acdisp", true) == 0)
		{
			if(PlayerInfo[playerid][pAdmin] == 20)
			{
                for(new c=1;c<MAX_HOUSES;c++)
				{
				    ...
				    {
				    	format(string, sizeof(string), "{878EE7}[INFO]:{C8C8C8} la pr�xima casa disponible para su colocaci�n es la: %d.", c);
		    ...
		    if(IsPlayerConnected(playerid))
		    {
				...
				if (PlayerInfo[playerid][pAdmin] == 20)
				{
					...
					if(id2 == 1)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: grande - 3 dormitorios", id,id2);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 2)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: mediana - 1 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
					else if(id2 == 3)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: peque�a - 1 dormitorio", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 4)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: muy grande - 4 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 5)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: peque�a - 2 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 6)
					{
     				...
     				else if(id2 == 9)
					{
						...
						House[id][ExitInterior] = 2;
						format(string, sizeof string, "Casa: %d - descripci�n: grande - 3 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     			 	else if(id2 == 10)
					{
						...
						House[id][ExitInterior] = 2;
						format(string, sizeof string, "Casa: %d - descripci�n: mediana - 2 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 11)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: grande - 4 dormitorios", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 12)
					{
         			...
         			else if(id2 == 14)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: mediana - 1 dormitorio", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 15)
					{
						...
						House[id][ExitInterior] = 4;
						format(string, sizeof string, "Casa: %d - descripci�n: grande - 1 dormitorio", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 16)
					{
     				...
     				else if(id2 == 19)
					{
						...
						House[id][ExitInterior] = 6;
						format(string, sizeof string, "Casa: %d - descripci�n: peque�a - ning�n dormitorio", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 20)
					{
						...
						House[id][ExitInterior] = 15;
						format(string, sizeof string, "Casa: %d - descripci�n: mediana - 1 dormitorio - 1 living room - 1 cocina", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 21)
					{
     				...
     				else if(id2 == 24)
					{
						...
						House[id][ExitInterior] = 1;
						format(string, sizeof string, "Casa: %d - descripci�n: Denise's Bedroom", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 25)
					{
						...
						House[id][ExitInterior] = 2;
						format(string, sizeof string, "Casa: %d - descripci�n: Katie's Bedroom", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
      		 		else if(id2 == 26)
					{
						...
						House[id][ExitInterior] = 3;
						format(string, sizeof string, "Casa: %d - descripci�n: Helena's Bedroom (barn) - limited movement.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 27)
					{
						...
						House[id][ExitInterior] = 4;
						format(string, sizeof string, "Casa: %d - descripci�n: Michelle's Bedroom.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 28)
					{
						...
						House[id][ExitInterior] = 5;
						format(string, sizeof string, "Casa: %d - descripci�n: Barbara's Bedroom.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 29)
					{
						...
						House[id][ExitInterior] = 6;
						format(string, sizeof string, "Casa: %d - descripci�n: Millie's Bedroom.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 30)
					{
						...
						House[id][ExitInterior] = 3;
						format(string, sizeof string, "Casa: %d - descripci�n: CJ's Mom's House.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 31)
					{
						...
						House[id][ExitInterior] = 5;
						format(string, sizeof string, "Casa: %d - descripci�n: Madd Dogg's Mansion (West door).", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 32)
					{
						...
						House[id][ExitInterior] = 2;
						format(string, sizeof string, "Casa: %d - descripci�n: Ryder's house.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 33)
					{
						...
						House[id][ExitInterior] = 1;
						format(string, sizeof string, "Casa: %d - descripci�n: Sweet's House (South side of house is fucked).", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 34)
					{
						...
						House[id][ExitInterior] = 2;
						format(string, sizeof string, "Casa: %d - descripci�n: Big Smoke's Crack Factory (Ground Floor).", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 35)
					{
						...
						House[id][ExitInterior] = 6;
						format(string, sizeof string, "Casa: %d - descripci�n: Fanny Batter's Whore House.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 36)
					{
						...
						House[id][ExitInterior] = 3;
						format(string, sizeof string, "Casa: %d - descripci�n: Tiger Skin Rug Brothel.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 37)
					{
						...
						House[id][ExitInterior] = 5;
						format(string, sizeof string, "Casa: %d - descripci�n: Burning Desire Gang House.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 38)
					{
						...
						House[id][ExitInterior] = 8;
						format(string, sizeof string, "Casa: %d - descripci�n: Colonel Furhberger's House.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 39)
					{
						...
						House[id][ExitInterior] = 5;
						format(string, sizeof string, "Casa: %d - descripci�n: Crack Den.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 40)
					{
						...
						format(string, sizeof string, "Casa: %d - descripci�n: Unused Safe House.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
     				else if(id2 == 41)
					{
						...
						House[id][ExitInterior] = 12;
						format(string, sizeof string, "Casa: %d - descripci�n: habitaci�n motel Budget Inn.", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
         			else if(id2 == 42)
					{
						...
						House[id][ExitInterior] = 15;
						format(string, sizeof string, "Casa: %d - descripci�n: Jefferson Motel. (REALLY EXPENSIVE)", id);
						SendClientMessage(playerid, COLOR_ADMINCMD,string);
     				}
					saveHouse(id);
				}
		...
		if(strcmp(cmd, "/acsalida", true) == 0)
		{
		    if(IsPlayerConnected(playerid))
		    {
				...
				if (PlayerInfo[playerid][pAdmin] == 20)
				{
					...
					saveHouse(id);
					new form[128];
					format(form, sizeof(form), "{878EE7}[INFO]:{C8C8C8} la salida de la casa %d ha sido cambiada a tu posici�n.", id);
					SendClientMessage(playerid, COLOR_ADMINCMD, form);
				}
			}
			return 1;
	 	...
	 	if(strcmp(cmd, "/acdesc", true) == 0)
		{
		    if(IsPlayerConnected(playerid))
		    {
				...
				if(!strlen(tmp))
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /acdesc [idcasa] [descripci�n]");
					return 1;
				}
				new id = strval(tmp);
				if (PlayerInfo[playerid][pAdmin] == 20)
				{
					...
					if(strfind( result , "|" , true ) == -1)
	    			{
						...
						format(string, sizeof(string), "{878EE7}[INFO]:{C8C8C8} la descripci�n de la casa %d ha sido cambiada a %s.", id,(result));
						SendClientMessage(playerid, COLOR_ADMINCMD, string);
						saveHouse(id);
					}
	 				else
					{
						SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} el s�mbolo | no est� permitido.");
					}
				}
			}
			return 1;
	...
	//=====================================================================================================================
		if(strcmp(cmd,"/toggle",true)==0)
		{
	 	if(IsPlayerConnected(playerid))
	 	{
			...
			if(strcmp(x_info,"gasoil",true) == 0)
			{
				...
				{
					SendClientMessage(playerid,COLOR_LIGHTYELLOW2, "Ya no ver�s informaci�n de la gasolina.");
					ShowFuel[playerid] = 0;
				}
				else
				{
				    SendClientMessage(playerid,COLOR_LIGHTYELLOW2, "Ahora ver�s informaci�n de la gasolina.");
				    ShowFuel[playerid] = 1;
				}
			}
	  		...
	  		else if(strcmp(x_info,"telefono",true) == 0)
			{
				...
				{
				    SendClientMessage(playerid,COLOR_LIGHTYELLOW2, "Has encendido tu tel�fono.");
		            PhoneOnline[playerid] = 0;
				}
				else
				{
	 	...
	 	if(strcmp(cmd, "/donar", true) == 0)
		{
		    if(IsPlayerConnected(playerid))
		    {
				...
				if(moneys < 0)
				{
					SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} cantidad inv�lida.");
					return 1;
				}
...
public OnBanDataLoad(playerid) {
	...
	if(rows) {
	    ...
	    SendClientMessage(playerid, COLOR_WHITE, "Para m�s informaci�n pasa por nuestros foros www.isamp.com.ar");
...
public OnPlayerNameCheck(playerid) {
	...
	if(rows == 0) {
		format(string, sizeof(string), "** %s (%d) ha iniciado sesi�n. IP: %s. Registrado: no. **", name, playerid, PlayerIP);
		AdministratorMessage(COLOR_GREY, string, 1);
		// Checkeamos si el nombre cumple con los requisitos.
		if(RPName(playerid)) {
			ShowPlayerDialog(playerid,DLG_REGISTER,DIALOG_STYLE_PASSWORD,"�Bienvenido! - nombre no registrado.","Para registrar esta cuenta, ingresa una contrase�a a continuaci�n: \r\n- Debe tener un m�nimo de 6 caracteres y no mas de 16.","Registar","");
		}
	} else {
...
public fuelCar(playerid, refillprice, refillamount, refilltype, validslot)
{
	if(refilltype == 1)
	{
		...
		SendFMessage(playerid, COLOR_WHITE, "El tanque de su veh�culo ha sido cargado al %d porciento por $%d.", VehicleInfo[GetPlayerVehicleID(playerid)][VehFuel], refillprice);
	} else
		if(refilltype == 2)
		{
		    setInvParam(playerid, validslot, getInvParam(playerid, validslot) + refillamount);
		    SendFMessage(playerid, COLOR_WHITE, "Has cargado nafta en tu bid�n de combustible al %d porciento por $%d.", getInvParam(playerid, validslot), refillprice);
	    }
...
public globalUpdate() {
	...
	foreach(new playerid : Player) {
		...
		if(gPlayerLogged[playerid] && PlayerInfo[playerid][pTutorial] == 1 && PlayerInfo[playerid][pRegStep] == 0) {
			...
			if(cAFK[playerid] == 0 && PlayerInfo[playerid][pJailed] != 2) // Si no est� AFK ni en Jail OOC
			{
							...
							foreach(new play : Player)
							{
				               	if(PlayerInfo[play][pFaction] == FAC_HOSP || (PlayerInfo[play][pFaction] == FAC_PMA && CopDuty[play]) )
								{
	                                if(PlayerInfo[play][pFaction] == FAC_PMA)
				               			SendClientMessage(play, COLOR_PMA, "[Llamado al 911]: �Atenci�n! Un ciudadano ha reportado a un herido de gravedad. Lo marcamos en su GPS.");
									else
									    SendClientMessage(play, COLOR_WHITE, "[Hospital]: �Atenci�n! Hemos marcado en su GPS la ubicaci�n de una llamada de emergencia que requiere asistencia.");
...
public OnPlayerStateChange(playerid, newstate, oldstate) {
	...
	if(newstate == PLAYER_STATE_ONFOOT) {
		if(SeatBelt[playerid])	{
			...
			if(vType == VTYPE_BIKE || vType == VTYPE_BMX || vType == VTYPE_QUAD) {
                ...
                PlayerActionMessage(playerid, 15.0, "se saca el cintur�n de seguridad.");
            }
	...
	if(newstate == PLAYER_STATE_DRIVER && oldstate == PLAYER_STATE_ONFOOT) {
		...
		if(GetVehicleType(vehicleid) != VTYPE_BMX) {
		    // Si no es una bicicleta mostramos el veloc�metro.
        ...
        if(VehicleInfo[vehicleid][VehType] == VEH_OWNED) {
			...
			if(playerLicense[playerid][lDTaking] != 1) {
				...
				SendClientMessage(playerid, COLOR_WHITE, "Enciende el motor (/motor) y conduce sobre los puntos del mapa respetando las leyes de tr�nsito.");
			...
			for(new i = 1; i < MAX_RENTCAR; i++)
			{
		 		if(RentCarInfo[i][rVehicleID] == vehicleid && RentCarInfo[i][rRented] == 0)
		 		{
					...
					if(price < 200)
						price = 200; // Seteamos un m�nimo de precio
...
public buyDrugsTimer(playerid, amount) {
	...
	if(!charged) {
		...
		SendClientMessage(playerid, COLOR_WHITE, "V� al punto de venta y tipea /comprar para intercambiar la materia prima por drogas.");
...
public buyProductsTimer(playerid, amount) {
	...
	if(!charged) {
	    ...
	    {
			SendClientMessage(playerid, COLOR_WHITE, "V� a tu negocio y descarga la nueva mercaderia dentro escribiendo /descargar en la entrada!");
			PlayerActionMessage(playerid, 15.0, "compra algunos productos y los carga en el veh�culo.");
		}
	return 1;
}
...
CMD:descargar(playerid, params[]) {
	...
	for(new biz = 0; biz < MAX_BUSINESS; biz++)
	{
		...
		{
 			if(!IsPlayerInAnyVehicle(playerid))
		    	return SendClientMessage(playerid, COLOR_YELLOW2, "�Debes estar en un veh�culo!");
		    if(VehicleInfo[vehicleid][VehType] != VEH_OWNED && VehicleInfo[vehicleid][VehType] != VEH_FACTION)
		    	return SendClientMessage(playerid, COLOR_YELLOW2, "Debes estar en un veh�culo con due�o o de facci�n.");
			new totalAmount = 0;
       		for(new i = 0; i < GetVehicleMaxTrunkSlots(vehicleid); i++)
			{
			    if(getTrunkItem(vehicleid, i) == 50)
				{
					...
					{
						SendClientMessage(playerid, COLOR_YELLOW2, "No puedes cargar mas mercaderia al negocio. El dep�sito est� lleno.");
						break;
					}
			...
			if (totalAmount > 0)
			{
				...
				PlayerActionMessage(playerid, 15.0, "comienza a descargar mercader�a en el dep�sito del negocio.");
				TogglePlayerControllable(playerid, false);
    ...
    if(factionid != FAC_NONE && FactionInfo[factionid][fType] == FAC_TYPE_ILLEGAL) {
        if(IsPlayerInAnyVehicle(playerid) && vehicleid == FactionInfo[factionid][fMissionVeh]) {
            if(PlayerToPoint(4.0, playerid, VehicleInfo[vehicleid][VehPosX], VehicleInfo[vehicleid][VehPosY], VehicleInfo[vehicleid][VehPosZ])) {
	            ...
	            SetPlayerCheckpoint(playerid, VehicleInfo[vehicleid][VehPosX], VehicleInfo[vehicleid][VehPosY], VehicleInfo[vehicleid][VehPosZ], 5.4);
				SendClientMessage(playerid, COLOR_WHITE, "V� a la siguiente localizaci�n para descargar la van.");
	        }
		} else {
		    SendClientMessage(playerid, COLOR_YELLOW2, "Debes estar dentro de la van de la facci�n.");
		}
    }
	return 1;
}
...
stock ReloadHouseIcon(houseid) {
	...
	if(House[houseid][Owned] == 0) {
	    ...
	    format(string, sizeof(string), "{49AFE6}Casa con due�o {FFFFFF}\nDirecci�n: %d %s\nPresiona ENTER para entrar", houseid, houseLoc);
...
// Obtenemos el par�metro adicional (EJ: balas).
...
// Seteamos el par�metro adicional.
    ...
    if(IsPlayerConnected(targetid)) {
		if(gPlayerLogged[targetid]) {
			...
			SendFMessage(playerid, COLOR_WHITE, "Salud: %.1f | Nivel: %d | Experiencia: %d/%d | Advertencias: %d", health, PlayerInfo[targetid][pLevel], PlayerInfo[targetid][pExp], (PlayerInfo[targetid][pLevel] + 1) * ServerInfo[svLevelExp], PlayerInfo[targetid][pWarnings]);
   			SendFMessage(playerid, COLOR_WHITE,	"Casa: %d | Negocio: %d | Veh�culos: %d/%d | Horas de juego: %d", PlayerInfo[targetid][pHouseKey], PlayerInfo[targetid][pBizKey], PlayerInfo[targetid][pVeh1], PlayerInfo[targetid][pVeh2], PlayerInfo[targetid][pPlayingHours]);
			if(admin) {
                ...
                SendFMessage(playerid, COLOR_WHITE, "Adicci�n: %.1f %% | Abstinencia en: %d min | Hambre: %.1f %% | Sed: %.1f %%", PlayerInfo[targetid][pAdictionPercent], PlayerInfo[targetid][pAdictionAbstinence] / 60, PlayerInfo[targetid][pHunger], PlayerInfo[targetid][pThirst]);
			}
...
public speedoTimer(playerid) {
	...
	new string2[64];
  	...
  	// Actualizamos la informaci�n del veloc�metro.
...
public Reparacion(playerid) {
	...
	if(GetPlayerCash(playerid) >= RepairPrice[playerid]) {
		...
		format(string, sizeof(string), "Su veh�culo ha sido reparado por %s (costo: $%d).", GetPlayerNameEx(RepairOffer[playerid]), RepairPrice[playerid]);
		SendClientMessage(playerid, COLOR_YELLOW2, string);
		format(string, sizeof(string), "Reparaste el veh�culo de %s, $%d han sido a�adidos a su PayDay.", GetPlayerNameEx(playerid), RepairPrice[playerid]);
            ...
            switch(Choice[playerid]) {
                case CHOICE_CARSELL: {
					...
					if(!IsAtDealership(playerid)) {
					    SendClientMessage(playerid, COLOR_YELLOW2, "�No puedes vender un veh�culo en cualquier lugar!");
					    return 1;
					}
					if(PlayerInfo[playerid][pVeh1] == vehicleid || PlayerInfo[playerid][pVeh2] == vehicleid) {
						...
						format(string, sizeof(string), "Empleado: has vendido tu veh�culo por $%d, que tenga un buen d�a.", price / 2);
				...
				switch(listitem)
				{
					...
					case 5:
					{
						...
						if(PlayerInfo[playerid][pPhoneBook] == 1)
						    return SendClientMessage(playerid, COLOR_YELLOW2, "Ya tienes una gu�a telef�nica.");
						GivePlayerCash(playerid, -PRICE_PHONEBOOK);
						PlayerActionMessage(playerid, 15.0, "le paga al empleado por una gu�a telef�nica.");
						SendFMessage(playerid, COLOR_WHITE, "�Has comprado una gu�a telef�nica por $%d!", PRICE_PHONEBOOK);
			        ...
			        case 6:
					{
						...
						SendFMessage(playerid, COLOR_WHITE, "Has comprado un bid�n por $%d, utiliza '/bidon' para m�s informaci�n.", PRICE_FUELCAN);
			     	...
			     	case 7:
					 {
						...
						SendFMessage(playerid, COLOR_WHITE, "Has comprado una c�mara fotogr�fica por $%d.", PRICE_CAMERA);
...
CMD:resetcars(playerid, params[]) {
	...
	if(sscanf(params, "s[128]", pass)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /resetcars [contrase�a de seguridad]");
...
CMD:ajail(playerid, params[]) {
	...
	if(PlayerInfo[playerid][pAdmin] < 1) return 1;
	if(sscanf(params,"uds[64]", targetID, minutes, reason)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /ajail [ID-Jugador] [minutos] [raz�n]");
	else if(targetID != INVALID_PLAYER_ID) {
		gettime(time[0], time[1], time[2]);
...
CMD:aeentrada(playerid, params[]) {
	...
	if(sscanf(params, "d", blid)) {
		SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /aeentrada [idedificio] - setea la entrada a tu posici�n, tu �ngulo ser� el adoptado al salir.");
	}
	else if(blid >= 1 && blid < MAX_BUILDINGS) {
...
CMD:aesalida(playerid, params[]) {
	...
	if(sscanf(params, "d", blid)) {
		SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /aesalida [idedificio] - setea la salida a tu posici�n, tu �ngulo ser� el adoptado al ingresar.");
	}
	else if(blid >= 1 && blid < MAX_BUILDINGS) {
...
CMD:aefaccion(playerid, params[]) {
	...
	if(sscanf(params, "dd", blid, faction)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /aefaccion [idedificio] [facci�n, 0 = ninguna]");
	else if(blid >= 1 && blid < MAX_BUILDINGS) {
		Building[blid][blFaction] = faction;
		format(string, sizeof(string), "Has seteado la facci�n del edificio [%d] a %d.", blid, faction);
		SendClientMessage(playerid, COLOR_ADMINCMD, string);
		saveBuilding(blid);
	}
	return 1;
...
CMD:reportar(playerid,params[])
{
	...
	if(sscanf(params,"us[128]", id, reporttext))
		return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8tC8} /reportar [ID/Jugador] [raz�n]");
	...
	if(maskCode != INVALID_PLAYER_ID)
	{
	    if(isUsingMaskInSlot[playerid] != -1)
	    {
			format(string, sizeof(string), "[Reporte]: %s ha reportado a %s (ID:%d), raz�n: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(maskCode), maskCode, reporttext);
			AdministratorMessage(COLOR_ADMINCMD, string, 1);
			format(string, sizeof(string), "Has reportado a %s (ID:%d), raz�n: %s", GetPlayerNameEx(maskCode), maskCode, reporttext);
			SendClientMessage(playerid, COLOR_WHITE, string);
		} else
	  		{
    ...
    if(sscanf(params, "s[128]I(-1)", text, returnid)) {
	    ...
	    for(new i = 0; i < INV_MAX_SLOTS; i++) {
	        ...
	        if(getItemType(itemid) == ITEM_WEAPON) {
	        	SendFMessage(playerid, COLOR_WHITE, " - %d- Arma: %s - Munici�n: %d", i, itemName[itemid], param);
...
CMD:ultveh(playerid, params[]) {
	...
	format(string, sizeof(string), "Ultimo veh�culo: %d", LastVeh[playerid]);
	SendClientMessage(playerid, COLOR_YELLOW2, string);
	return 1;
}
...
CMD:ayuda(playerid,params[]) {
	...
	SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FFDD00}[Tel�fono]:{C8C8C8} /llamar /servicios /atender /colgar /sms /numero");
	SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FFDD00}[Propiedades]:{C8C8C8} /ayudacasa /ayudanegocio /ayudabanco /ayudacajero");
	SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FFDD00}[Veh�culo]:{C8C8C8} /motor (/veh)iculo /maletero (/cin)tur�n (/cas)co /emisora /sacar /ventanillas");
    ...
    if(PlayerInfo[playerid][pFaction] != 0)
	{
		...
		if(PlayerInfo[playerid][pFaction] == FAC_PMA) {
		    ...
		    if(PlayerInfo[playerid][pRank] == 1) {
		        SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"{FFDD00}[L�der]:{C8C8C8} /comprar (en el shop de materiales) /descargar /ensamblar (dentro del HQ) /ch /it /ru");
		    } else {
...
CMD:atender(playerid, params[]) {
	...
	if(eventStep[playerid] == 1) {
	    SendClientMessage(playerid, COLOR_FADE1, "An�nimo dice: nos han llegado nuevas partes, �deseas ir a recogerlas? recibir�s adem�s $1000 en mano.");
...
CMD:msg(playerid, params[]) {
	...
	if(sscanf(params, "ds[128]", phonenumber, text)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /msg [n�mero telef�nico] [texto]");
	else {
		if(PlayerInfo[playerid][pPhoneNumber] == 0) {
			SendClientMessage(playerid, COLOR_YELLOW2, "�No tienes un tel�fono celular! consigue uno en un 24/7.");
			return 1;
		}
			...
			foreach(new i : Player) {
				if(PlayerInfo[i][pPhoneNumber] == phonenumber && phonenumber != 0) {
					if(IsPlayerConnected(i) && i != INVALID_PLAYER_ID) {
				        if(PhoneOnline[i]) {
				            SendClientMessage(playerid, COLOR_YELLOW2, "El tel�fono al que te quieres contactar no se encuentra en servicio.");
				            return 1;
				        }
...
CMD:colgar(playerid, params[]) {
	...
	if(IsPlayerConnected(caller) && caller != INVALID_PLAYER_ID && caller != 255) {
		...
		if(caller < 255) {
			...
			PlayerActionMessage(playerid,15.0,"guarda su tel�fono celular en el bolsillo.");
			PlayerActionMessage(caller,15.0,"guarda su tel�fono celular en el bolsillo.");
			Mobile[caller] = 255;
...
CMD:llamar(playerid, params[]) {
    ...
    if(sscanf(params, "d", number)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /llamar [n�mero de tel�fono]");
	else {
		if(PlayerInfo[playerid][pPhoneNumber] == 0) {
			SendClientMessage(playerid, COLOR_YELLOW2, "�No puedes realizar una llamada si no tienes un tel�fono!");
			return 1;
		}
		if(Mobile[playerid] != 255) {
		...
		if(number == 444) {
			...
			if(workers < 1) {
			    SendClientMessage(playerid, COLOR_WHITE, "Telefonista: lo sentimos, no hay veh�culos disponibles por el momento.");
			    return 1;
			}
		...
		if(number == PlayerInfo[playerid][pPhoneNumber]) {
			SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} la l�nea est� siendo utilizada.");
			return 1;
	    }
	    // ----
		foreach(new giveplayerid : Player) {
			if(PlayerInfo[giveplayerid][pPhoneNumber] == number && number != 0) {
				...
				if(!IsPlayerConnected(giveplayerid) || giveplayerid == INVALID_PLAYER_ID) {
					SendClientMessage(playerid, COLOR_WHITE, "Operadora dice: no se ha podido contactar el n�mero que ha marcado.");
					return 1;
				}
		        if(PhoneOnline[giveplayerid] || PlayerInfo[giveplayerid][pSpectating] != INVALID_PLAYER_ID) {
		            SendClientMessage(playerid, COLOR_WHITE, "Operadora dice: el tel�fono al que intenta comunicarse se encuentra fuera de l�nea.");
		            return 1;
		        }
				if(Mobile[giveplayerid] == 255) {
					PlayerDoMessage(giveplayerid, 15.0, "Un tel�fono ha comenzado a sonar.");
...
CMD:numero(playerid, params[]) {
    ...
    if(sscanf(params, "u", targetID)) SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /numero [playerid/ParteDelNombre]");
	    ...
	    if(!IsPlayerConnected(targetID) || targetID == INVALID_PLAYER_ID || !gPlayerLogged[targetID]) {
	        SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{FF4600}[Error]:{C8C8C8} ID de jugador incorrecta o no ha iniciado sesi�n.");
	        return 1;
	    }
	    PlayerActionMessage(playerid, 15.0, "toma su guia telef�nica y busca el n�mero de una persona.");
 		if(PlayerInfo[targetID][pListNumber] == 0 || PlayerInfo[targetID][pPhoneNumber] == 0) {
            PlayerDoMessage(playerid, 15.0, "El n�mero buscado no se encuentra en la gu�a.");
			return 1;
		}
		SendFMessage(playerid, COLOR_WHITE, "Nombre: %s, n�mero de tel�fono: %d.", GetPlayerNameEx(targetID), PlayerInfo[targetID][pPhoneNumber]);
	} else {
		SendClientMessage(playerid, COLOR_YELLOW2, "�Debes tener una gu�a telef�nica!");
		return 1;
	}
	return 1;
}
	...
	if(business != 0 && Business[business][bType] == BIZ_247) {
	    ...
	    if(PlayerInfo[playerid][pWepLic]) {
			...
			if(sscanf(params, "dd", weapon, ammo)) {
				SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /comprar [n�mero] [balas]");
				SendFMessage(playerid, COLOR_LIGHTYELLOW2, " 1 - Pistola 9mm. - $%d por munici�n", itemPrice[22]);
				SendFMessage(playerid, COLOR_LIGHTYELLOW2, " 2 - Desert Eagle - $%d por munici�n", itemPrice[24]);
				SendFMessage(playerid, COLOR_LIGHTYELLOW2, " 3 - Escopeta - $%d por munici�n", itemPrice[25]);
				SendFMessage(playerid, COLOR_LIGHTYELLOW2, " 4 - Rifle de caza - $%d por munici�n", itemPrice[33]);
				...
				if(ammo > 0 && ammo <= 900) {
					...
					Business[business][bTill] += totalPrice / 2; // la mitad para evitar abusos de sacar armas gratis para el due�o
					Business[business][bProducts]--;
					saveBusiness(business);
				} else {
			    	SendClientMessage(playerid, COLOR_YELLOW2, "No puedes comprar menos de 1 bala ni m�s de 900.");
				}
			} else {
			    SendClientMessage(playerid, COLOR_YELLOW2, "N�mero de arma incorrecto, solo puedes de 1 a 4.");
			}
		} else {
		    SendClientMessage(playerid, COLOR_YELLOW2, "No tienes una licencia de portaci�n de armas.");
		}
		...
		{
			SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /comprar [n�mero]");
		...
		if(weapon < 1 || weapon > 10)
			return SendClientMessage(playerid, COLOR_YELLOW2, "Ingresa un numero de opcion v�lido.");
        switch(weapon)
		{
			case 1: realWeapon = 1;
			case 2: realWeapon = 6;
		...
		if(PlayerToPoint(4.0, playerid, 2333.2856, -1948.3102, 13.5783)) {
			if(PlayerInfo[playerid][pJob] == JOB_DRUGD) {
			    /* Tengo que optimizar y reescribir todo este c�digo. */
				new
				    amount,
					type,
					vehicleid = GetPlayerVehicleID(playerid);
				...
				if(sscanf(params, "dd", type, amount)) {
			        ...
			        SendClientMessage(playerid, COLOR_YELLOW2, "3: �xtasis          -    3 grs. por cada bolsa de materia prima.");
					...
					switch(type) {
					    ...
					    case 3: {
							...
							SendFMessage(playerid, COLOR_LIGHTBLUE, "Has comprado %d dosis de �xtasis por %d bolsas de materia prima.", 3 * result, result);
					    }
						case 4: {
							PlayerInfo[playerid][pCocaine] += 2 * result;
							SendFMessage(playerid, COLOR_LIGHTBLUE, "Has comprado %d gramos de coca�na por %d bolsas de materia prima.", 2 * result, result);
						}
     				}
			    } else {
			        SendClientMessage(playerid, COLOR_YELLOW2, "Tipo inv�lido, elige uno de los siguientes:");
			        SendClientMessage(playerid, COLOR_YELLOW2, "1: marihuana    -    5 grs. por cada bolsa de materia prima.");
			        SendClientMessage(playerid, COLOR_YELLOW2, "2: LSD   	           -    4 grs. por cada bolsa de materia prima.");
			        SendClientMessage(playerid, COLOR_YELLOW2, "3: �xtasis          -    3 grs. por cada bolsa de materia prima.");
			        SendClientMessage(playerid, COLOR_YELLOW2, "4: coca�na         -    2 grs. por cada bolsa de materia prima.");
		    ...
		    if(PlayerInfo[playerid][pJob] == JOB_DRUGD) {
		        ...
		        if(!IsPlayerInAnyVehicle(playerid))
		            return SendClientMessage(playerid, COLOR_YELLOW, "�Debes estar en un veh�culo!");

		        if(VehicleInfo[vehicleid][VehType] != VEH_OWNED && VehicleInfo[vehicleid][VehType] != VEH_FACTION)
		            return SendClientMessage(playerid, COLOR_YELLOW, "Debes estar en un veh�culo con due�o o de facci�n.");
			...
			if(PlayerInfo[playerid][pFaction] != FAC_NONE && FactionInfo[PlayerInfo[playerid][pFaction]][fType] == FAC_TYPE_ILLEGAL) {
			    ...
			    if(sscanf(params, "d", weapon)) {
				    ...
				    SendClientMessage(playerid, COLOR_YELLOW2, "N�mero de arma incorrecto, solo puedes de 1 a 3.");
				}
...
CMD:clasificado(playerid,params[])
{
	...
	if(gPlayerLogged[playerid] != 1)
		return 1;
    if(sscanf(params, "s[128]", text))
		return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} (/cla)sificado [texto - 64 car�cteres] $280");
...
CMD:rentar(playerid, params[])
{
	...
	if(price < 300)
		price = 300; // Seteamos un m�nimo de precio
	if(GetPlayerCash(playerid) < price * time)
    	return SendClientMessage(playerid, COLOR_YELLOW2, "No tienes el dinero necesario.");

	GivePlayerCash(playerid, -(price * time));
	SendClientMessage(playerid, COLOR_WHITE, "�Has rentado este veh�culo! Utiliza /motor para encenderlo. El veh�culo ser� devuelto al acabarse el tiempo.");
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)	{
		GetVehicleParamsEx(vehicleid, VehicleInfo[vehicleid][VehEngine], VehicleInfo[vehicleid][VehLights], VehicleInfo[vehicleid][VehAlarm], vlocked, VehicleInfo[vehicleid][VehBonnet], VehicleInfo[vehicleid][VehBoot], VehicleInfo[vehicleid][VehObjective]);
	 	if(VehicleInfo[vehicleid][VehType] == VEH_DEALERSHIP || VehicleInfo[vehicleid][VehType] == VEH_DEALERSHIP2 || VehicleInfo[vehicleid][VehType] == VEH_SHIPYARD) {
		    ...
		    SendClientMessage(playerid, COLOR_YELLOW2, "El veh�culo no tiene combustible.");
		    return 1;
		} else if(VehicleInfo[vehicleid][VehHP] < 500) {
			PlayerActionMessage(playerid,15.0,"intenta encender el motor del veh�culo pero se encuentra da�ado.");
			SendClientMessage(playerid, COLOR_YELLOW2, "El veh�culo se encuentra averiado.");
			return 1;
		    ...
		    SendClientMessage(playerid, COLOR_WHITE, "Para encender esta veh�culo utiliza /trabajar.");
		    return 1;
			...
			PlayerActionMessage(playerid,15.0,"ha apagado el motor del veh�culo.");
			SetEngine(vehicleid, 0);
		}
	} else {
	    SendClientMessage(playerid,COLOR_LIGHTYELLOW2,"{FF4600}[Error]:{C8C8C8} debes estar en un veh�culo para utilizar este comando.");
	}
	return 1;
}
...
CMD:pservicio(playerid, params[])
{
	...
	if(CopDuty[playerid] == 0) {
		PlayerActionMessage(playerid,15.0,"se coloca el uniforme de polic�a y toma sus herramientras del armario.");
		...
		PlayerActionMessage(playerid, 15.0, "se quita el uniforme de polic�a y guarda sus herramientras en el armario.");
		ResetPlayerWeapons(playerid);
		    ...
		    switch(id) {
		        case 1: {
                    if(PlayerInfo[playerid][pRank] <= 8) {
						...
						GivePlayerWeapon(playerid, 43, 20); // C�mara
				...
				case 5: {
                    if(PlayerInfo[playerid][pRank] <= 3) {
		                ResetPlayerWeapons(playerid);
		                GivePlayerWeapon(playerid, 43, 50); // C�mara
		            	GivePlayerWeapon(playerid, 4, 1); // Cuchillo
						GivePlayerWeapon(playerid, 24, 60); // DEagle
		            } else {
		    ...
		    switch(id) {
		        ...
		        case 9: {
		        	if(PlayerInfo[playerid][pRank] <= 6 && GEOF == 1) {
		            	...
		            	SendFMessage(playerid, COLOR_YELLOW2, "Debes ser un %s para tener acceso a este equipo o tener autorizaci�n a G.E.O.F.", GetRankName(1, 6));
						return 1;
		            }
		        }
		        case 10: {
		        	if(PlayerInfo[playerid][pRank] <= 5 && GEOF == 1) {
		            	...
		            	SendFMessage(playerid, COLOR_YELLOW2, "Debes ser un %s para tener acceso a este equipo o tener autorizaci�n a G.E.O.F.", GetRankName(FAC_PMA, 5));
						return 1;
		            }
		        }
		        case 11: {
		        	if(PlayerInfo[playerid][pRank] <= 4 && GEOF == 1) {
		            	...
		            	SendFMessage(playerid, COLOR_YELLOW2, "Debes ser un %s para tener acceso a este equipo o tener autorizaci�n a G.E.O.F.", GetRankName(FAC_PMA, 4));
						return 1;
		            }
		        }
		        case 12: {
...
stock ShowInv(playerid, targetid)
{
    ...
    for(new x = 0; x < INV_MAX_SLOTS; x++) {
        ...
        if(getItemType(itemid) == ITEM_WEAPON) {
	       	SendFMessage(playerid, COLOR_WHITE, " - %d- Arma: %s - Munici�n: %d", x, itemName[itemid], param);
		} else
...
CMD:mostrarced(playerid, params[])
{
	...
	{
		SendClientMessage(targetid, COLOR_LIGHTGREEN, "================[C�dula de Identificaci�n del Automotor]===============");
    	...
    	return SendClientMessage(playerid, COLOR_YELLOW2, "No tienes la c�dula de ese veh�culo, �No es tuyo!");
	return 1;
}
...
public UpdatePlayerAdiction()
{
    foreach(new playerid : Player)
    {
    	if(cAFK[playerid] == 0 && PlayerInfo[playerid][pJailed] != 2 && AdminDuty[playerid] != 1) // Si no est� AFK ni en Jail OOC
		{
			    	...
			    	if(PlayerInfo[playerid][pAdictionAbstinence] > 0)
					{
			        	...
			        	if(PlayerInfo[playerid][pAdictionAbstinence] <= 0.0)
					...
					} else
		   				{
							...
							SendClientMessage(playerid, COLOR_RED, "Luego de un tiempo logran salvarte, cobr�ndote $20.000. Si no lo tienes se descontar� de tu cuenta bancaria.");
...
CMD:aceptar(playerid,params[]) {
	...
	if(sscanf(params, "s[64]", text)) {
		...
		// Comprobamos que el recipiente tenga un slot libre, si lo tiene realizamos la transacci�n.
		if(PlayerInfo[playerid][pVeh1] <= 0) {
		    // Seteamos las variables del veh�culo con los datos del nuevo due�o.
			...
			// Se lo seteamos a la cuenta del nuevo due�o y realizamos la transacci�n de dinero.
			...
			SendClientMessage(playerid, COLOR_LIGHTBLUE, "No puedes tener m�s veh�culos, cancelando...");
		    SendClientMessage(VehicleOffer[playerid], COLOR_LIGHTBLUE, "El jugador no puede tener m�s veh�culos, cancelando...");
		}
		KillTimer(GetPVarInt(playerid, "CancelVehicleTransfer"));
		...
		if(RepairOffer[playerid] == 999)
			return SendClientMessage(playerid, COLOR_YELLOW2, "�Nadie te ha ofrecido una reparaci�n!");
		if(GetPlayerCash(playerid) < RepairPrice[playerid])
			return SendClientMessage(playerid, COLOR_YELLOW2, "�No tienes el dinero suficiente para pagar la reparaci�n!");
        ...
        if(TaxiCall < 999) {
            if(IsPlayerConnected(TaxiCall)) {
            	format(string, sizeof(string), "* Has aceptado la llamada de %s, ver�s un marcador en el GPS.", GetPlayerNameEx(TaxiCall));
				SendClientMessage(playerid, COLOR_WHITE, string);
		...
		if(TuningOffer[playerid] == 999)
   		...
   		if(!ProxDetectorS(5.0, playerid, TuningOffer[playerid]))
  	    	return SendClientMessage(playerid, COLOR_WHITE, "El mec�nico no est� cerca tuyo.");
		if(!IsPlayerConnected(TuningOffer[playerid]))
		    return SendClientMessage(playerid, COLOR_WHITE, "El mecanico no est� conectado.");
	    ...
	    if(!IsPlayerConnected(DestuningOffer[playerid]))
		    return SendClientMessage(playerid, COLOR_WHITE, "El mecanico no est� conectado.");
	    if(!IsPlayerInAnyVehicle(playerid))
			return SendClientMessage(playerid, COLOR_WHITE, "No est�s en un ning�n vehiculo.");
		new vID = GetPlayerVehicleID(playerid);
	    if(GetVehicleType(vID) != VTYPE_CAR)
	    	return SendClientMessage(playerid, COLOR_WHITE, "Vehiculo invalido.");
	    if(PlayerInfo[playerid][pVeh1] != vID && PlayerInfo[playerid][pVeh2] != vID)
			return SendClientMessage(playerid, COLOR_WHITE, "No eres due�o de ese vehiculo.");
...
CMD:comprarlic(playerid,params[]) {
	if(PlayerToPoint(1.0, playerid, 249.3701, 71.1788, 1003.6406) && GetPlayerVirtualWorld(playerid) == 16002) {
		...
		SendClientMessage(playerid, COLOR_YELLOW2, "�Debes estar en el lugar correcto de la comisar�a!");
	}
	return 1;
}
...
CMD:dar(playerid, params[])
{
    ...
    if(GetPlayerState(playerid) == 3)
        return SendClientMessage(playerid, COLOR_YELLOW2, "No puedes utilizar este comando como pasajero! sal del veh�culo primero.");
	if((PlayerInfo[playerid][pFaction] == FAC_PMA && CopDuty[playerid]) || (PlayerInfo[playerid][pFaction] == FAC_SIDE && SIDEDuty[playerid]))
	...
	pWeapon = GetPlayerWeapon(playerid);
 	pAmmo = GetPlayerAmmo(playerid);
  	if(!isWeaponAllowed(pWeapon))
        return SendClientMessage(playerid, COLOR_YELLOW2, "Arma inv�lida (/ban ^^).");
	if(pWeapon <= 0)
	...
	if(IsPlayerInAnyVehicle(playerid)) {
	    vehicleid = GetPlayerVehicleID(playerid);
	   	if(GetVehicleType(vehicleid) == VTYPE_CAR) {
			...
			SendClientMessage(playerid, COLOR_YELLOW2,"�Este veh�culo no tiene ventanillas!");
		}
	} else {
		SendClientMessage(playerid, COLOR_YELLOW2,"�Debes estar en un veh�culo para utilizar este comando!");
	}
	return 1;
}
			...
			if(GetPlayerVirtualWorld(playerid) == i + 17000)
			{
	    		if(Business[i][bType] == BIZ_REST)
				{
					...
					if(sscanf(params, "i", menu))
					{
						...
						SendClientMessage(playerid, COLOR_WHITE, "4) - Ensalada C�esar - Precio: $70.");
					...
					switch(menu)
					{
						...
						case 4:
					    {
					        if(GetPlayerCash(playerid) < 70)
	       		    		...
	       		    		Business[i][bTill] += 70;
	            	   		Business[i][bProducts]--;
					  		PlayerActionMessage(playerid, 15.0, "ordena una ensalada Ca�sar y comienza a comer.");
						...
						default:
							SendClientMessage(playerid, COLOR_YELLOW2, "�Ese men� no se encuentra en la carta, ordena alguno de los disponibles!");
					}
				}
			}
		}
						...
						if(strcmp(command, "cerveza", true) == 0) {
							...
							if(GetPlayerCash(playerid) >= 25)
							{
		       					GivePlayerCash(playerid, -25);
		             	       	Business[i][bTill] += 25;
		                	    Business[i][bProducts]--;
						  		PlayerActionMessage(playerid, 15.0, "se pide una taza de caf� y lo bebe.");
						  		PlayerDrink(playerid, 50.0);
						  		saveBusiness(i);
							} else {
							    SendClientMessage(playerid, COLOR_YELLOW2, "�Vuelve cuando tengas el dinero suficiente!");
		...
		if(PlayerToPoint(2.0, playerid, House[h][EntranceX], House[h][EntranceY], House[h][EntranceZ]) && House[h][Owned] == 0) {
		    if(House[h][HousePrice] > 0) {
				if(PlayerInfo[playerid][pHouseKey] == 0) {
					...
					SendClientMessage(playerid, COLOR_YELLOW2, "�No puedes tener m�s de una casa!");
				}
		    } else {
		    	SendClientMessage(playerid, COLOR_YELLOW2, "�Esta casa no tiene precio!, probablemente no est� a la venta.");
		    }
		}
    }
	return 1;
...
CMD:casavendera(playerid, params[])
{
    ...
    if(strcmp(House[house][Owner], name, true) == 0)
    {
	    ...
	    SendClientMessage(playerid, COLOR_YELLOW2, "Error, la casa no est� a tu nombre (Consulta a un administrador).");
	return 1;
}
...
CMD:reparar(playerid,params[])
{
 	...
 	if(price < 0 || price > JOB_MECH_MAXPRICE)
	{
 	...
 	if(!ProxDetectorS(8.0, playerid, target) || !IsPlayerInAnyVehicle(target))
 	    return SendClientMessage(playerid, COLOR_YELLOW2, "{FF4600}[Error]:{C8C8C8} el jugador no se encuentra cerca tuyo o no est� dentro de un vehiculo.");
	...
	SendFMessage(target, COLOR_LIGHTBLUE, "El mec�nico %s le ha ofrecido reparar su veh�culo por {D30000}$%d{33CCFF}, escribe /aceptar reparacion para aceptar.", GetPlayerNameEx(playerid), price);
...
CMD:destunear(playerid, params[])
{
	new target;
 	...
 	if(PlayerInfo[playerid][pFaction] != FAC_MECH)
  		return 1;
    if(sscanf(params, "u", target))
		return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /destunear [idDue�o]");
 	if(!jobDuty[playerid])
	    return SendClientMessage(playerid, COLOR_YELLOW2, "Debes estar en servicio");
...
CMD:tunear(playerid,params[])
{
    ...
    if(PlayerInfo[playerid][pFaction] != FAC_MECH)
        return 1;
	if(sscanf(params, "u", target))
		return SendClientMessage(playerid, COLOR_LIGHTYELLOW2, "{5CCAF1}[Sintaxis]:{C8C8C8} /tunear [idDue�o]");
	if(!jobDuty[playerid])
	    return SendClientMessage(playerid, COLOR_YELLOW2, "Debes estar en servicio");
 	if(PlayerInfo[playerid][pRank] > 4)
    	return SendClientMessage(playerid, COLOR_YELLOW2, "No tienes el rango suficiente.");
...
CMD:emisora(playerid, params[])
{
	...
	if(redio < 1 || redio > 14)
	    return SendClientMessage(playerid, COLOR_YELLOW2, "Debes ingresar una radio v�lida: del 1 al 14.");
...
CMD:mascara(playerid, params[])
{
	...
	if(isUsingMaskInSlot[playerid] == -1)
	{
        ...
        PlayerActionMessage(playerid, 15.0, "agarra un pa�uelo de su bolsillo y se la coloca en la cara para tapar su rostro.");
	...
	} else
		{
			...
			PlayerActionMessage(playerid, 15.0, "quita el pa�uelo que ocultaba su rostro y lo guarda en su bolsillo.");
			isUsingMaskInSlot[playerid] = -1;
		    foreach(new i:Player)
		    {
...
CMD:comprarmascara(playerid, params[])
{
    ...
    if(business == 0 || (Business[business][bType] != BIZ_CLOT &&  Business[business][bType] != BIZ_CLOT2))
        return SendClientMessage(playerid, COLOR_YELLOW2, "Para comprar un pa�uelo debes dirigirte a algun negocio de ropa que los venda.");
	...
	if(Business[business][bProducts] < 1)
 	...
 	PlayerActionMessage(playerid, 15.0, "le paga al empleado $300 y compra un pa�uelo, que luego guarda en su bolsillo.");
...
CMD:cambiarnombre(playerid, params[]) {
	...
	if(sscanf(params, "us[24]", target, name)) {
	    ...
	    SendClientMessage(playerid, COLOR_YELLOW2, "Jugador inv�lido.");
	}
	return 1;
}
...
CMD:bidon(playerid, params[]) {
	...
	SendClientMessage(playerid, COLOR_WHITE, "situarte junto al tanque del veh�culo y tipear '/usarbidon', el estado del bid�n lo puedes ver usando '/inv'.");
	return 1;
}
...
CMD:cinturon(playerid, params[]) {
	...
	if(!SeatBelt[playerid]) {
		SeatBelt[playerid] = true;
		PlayerActionMessage(playerid, 15.0, "se pone el cintur�n de seguridad.");
	} else {
		SeatBelt[playerid] = false;
		PlayerActionMessage(playerid, 15.0, "se saca el cintur�n de seguridad.");
	}
...
CMD:consumir(playerid, params[]) {
	...
	if(sscanf(params, "s[32]", type)) {
		...
		SetPlayerWeather(playerid, weathers[random(4)]);
		SetPVarInt(playerid, "drugEffect", 600);
		PlayerActionMessage(playerid, 15.0, "toma una pastilla de �xtasis.");
		...
		SetPlayerWeather(playerid, weathers[random(4)]);
		SetPVarInt(playerid, "drugEffect", 600);
		PlayerActionMessage(playerid, 15.0, "aspira algo de coca�na mediante un tubo de papel.");
...
CMD:llenar(playerid, params[])
{
	...
	if(GetPlayerCash(playerid) < 6) // Lo m�nimo para llenar
	   	return SendClientMessage(playerid, COLOR_YELLOW2, "Vuelve cuando tengas el dinero suficiente.");
    ...
    if(IsPlayerInAnyVehicle(playerid))
	{
	...
	} else
		{
			...
			if(validslot == -1)
   				return SendClientMessage(playerid, COLOR_YELLOW2, "Debes estar dentro de un veh�culo o tener un bid�n con nafta para utilizar este comando.");
			else
			{
				refilltype = 2;
